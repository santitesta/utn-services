    ...
    <div className='w-1/6 grid justify-items-center content-start'>
      ...
      {localStorage.institute === 'Admin' ?
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='mt-3 grid justify-items-center content-start'>
          ...
          <div class="p-2 mb-2 w-48 text-sm text-primary bg-blue-100 rounded-lg shadow-lg hover:bg-blue-200">
            <span class="text-sm font-bold text-primary">Filtrar por N° de UTN</span>
            <div className='relative mt-3' >
              <input type="number"
                id="utn"
                className="block h-8 px-2.5 pb-2.5 pt-4 w-44 max-w-xs text-sm text-gray-900 bg-white rounded-lg border border-gray-500 
            ...
            scale-75 top-2 z-10 origin-[0] peer-enabled:bg-white disabled:text-lg bg-transparent dark:bg-gray-900 px-2 peer-focus:px-2
          ...
          <div class="mt-4 p-2 mb-2 w-48 text-sm text-primary bg-green-100 rounded-lg shadow-lg hover:bg-green-200">
            <span class="text-sm font-bold text-primary">Filtrar por ubicación</span>
          ...
          <div class="mt-4 p-2 mb-2 w-48 text-sm text-primary bg-yellow-100 rounded-lg shadow-lg hover:bg-yellow-200">
            <span class="text-sm font-bold text-primary">Filtrar por tipo de equipo</span>
            <select
              className="mt-3 form-select h-8 block w-44 px-3 py-1 text-sm text-gray-500 bg-white border border-solid
            border-slate-300 rounded-lg transition ease-in-out shadow-sm  focus:text-gray-700 focus:ring-0 focus:border-blue-600
            disabled:bg-gray-300"
              ...
              <option defaultValue="" value="">Muy pronto...</option>
              {/* {Object.keys(institutes).map(i => {
              return institutes[i] !== 'Admin' && <option key={i} value={institutes[i]}>{institutes[i]}</option>
            })} */}
            </select>
